...
import 'package:spa_manager_flutter/provider/services/components/dropdown_subcategory_component.dart';
...
class AddServiceScreenState extends State<AddServiceScreen> {
  ...
  List<String> typeList = [SERVICE_TYPE_FIXED, SERVICE_TYPE_HOURLY, SERVICE_TYPE_FREE];
  List<String> statusList = [SERVICE_TYPE_ACTIVE, SERVICE_TYPE_INACTIVE];
  ...
  getEditServiceData() {
    ...
    priceCont.text = serviceDetail.price.toString().validate(value: '0');
    ...
    serviceStatus = serviceDetail.status == 1 ? context.translate.lblActivate : context.translate.lblDeactivate;
    serviceType = serviceDetail.type.validate();
    serviceStatus = serviceDetail.status == 0 ? context.translate.lblDeactivate : context.translate.lblActivate;
  ...
  Future<void> getCategory() async {
    await getCategoryList(perPage: '?per_page=all').then((value) {
      ...
      if (isUpdate) selectedCategory = categoryList.where((element) => element.id == widget.data!.categoryId).first;
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        appBar: appBarWidget(
          isUpdate ? context.translate.lblEditService : context.translate.hintAddService,
          textColor: white,
                  ...
                  HorizontalList(
                      ...
                      itemBuilder: (context, i) {
                        return Stack(
                          ...
                          children: [
                            Image.file(File(imageFiles[i].path), width: 90, height: 90, fit: BoxFit.cover),
                            Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          DropdownSubCategoryComponent(
                            isValidate: false,
                            selectedValue: isUpdate ? widget.data!.subCategoryId.validate() : null,
