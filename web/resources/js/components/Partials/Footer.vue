...
<footer class="footer py-3">
        ...
        <div class="row footer-main-border">
             ...
             <div class="col-lg-6" v-for="(column,i) in columns" :key="i">
                <div class="extra-content pb-3">
                  <a v-for="(item,index) in column"  :key="index" class="extra-page-content" href="#">
                    <span><router-link :to="{name: 'service-detail',params: { service_id: item.id }}">{{item.name}}</router-link></span>
                  </a>
                </div>
            </div>
            </div>
          ...
          <div class="col-md-6 col-lg-3 col-xl-3 mb-4">
            <div class="card download-card">
              <div class="card-body">
                <h6 class="text-white mb-3">{{appsetting.title}}</h6>
                <span class="download-text">{{appsetting.description}}</span>
    ...
    <cookie-law theme="dark-lime" message="Trang web này sử dụng cookie để đảm bảo bạn có được trải nghiệm tốt nhất trên trang web của chúng tôi." buttonText="Chấp nhận"></cookie-law>
</footer>
</template>
<script>
